...
scalar sector is the general lowest-derivative polynomial in the standard
...
refractive source--index chain, and the Solar branch coincides with general
relativity through second post-Newtonian order: $\gamma=\beta=1$, vanishing
preferred-frame source combination at the order displayed, and
...
sources the exponential (Papapetrou) exterior exactly, reproducing its Einstein
tensor component by component without a propagating ghost --- a geometry
previously realized in general relativity only by a phantom scalar. A
$C^2$-matched regular core removes the wormhole throat and renders the
geometry horizonless and geodesically complete, and the deficit-feedback
...
is sharply falsifiable: the shadow radius exceeds the Schwarzschild value by
...
frontier is the strong static field: horizon-scale images of M87* and
Sagittarius~A* \cite{EHT2019M87,EHT2022SgrA} and the ringdown phase of binary
mergers now probe the geometry within a few Schwarzschild radii, where the
black-hole paradigm faces its sharpest alternatives --- horizonless
ultracompact objects whose exterior mimics a black hole while their interior
remains regular \cite{CardosoPani2019}. Quantitative tests of this kind
require concrete theories: actions whose field equations select a definite
exterior, rather than metric ansatze tuned to the data
...
is a theory of this class: one shift-symmetric phase scalar $\Phi$ and three
solid scalars $\phi^A$ coupled minimally to the metric --- the field content
...
light, and Newton's law emerges as the weak spherical limit of this
source--index chain.

The strong-field side of the paper concerns a geometry with a long history:
the exponential metric
$ds^2=e^{-r_s/r}c^2dt^2-e^{r_s/r}(dr^2+r^2d\Omega^2)$, first written by
Papapetrou \cite{Papapetrou1954}, later the centerpiece of Yilmaz's
gravitational theory \cite{Yilmaz1958} and of the ensuing controversy
\cite{Misner1999,AlleyYilmaz1995}, and recently re-examined within GR as a
traversable wormhole by Boonserm, Ngampitipan, Simpson, and Visser
...
with Event Horizon Telescope data in \cite{Vagnozzi2023}. We emphasize at the
outset that none of the exterior geodesic numbers quoted in this paper are
new; we use them with attribution and a one-line normalization table.

What has remained unsettled is the source. Within GR the exponential exterior
requires an effective stress whose radial null energy condition is violated at
every radius, and its canonical realization is a massless ghost (phantom)
scalar \cite{Boonserm2018}; the alternative realizations in the literature ---
antiscalar backgrounds \cite{MakukovMychelkin2018}, Yilmaz's own field
equations \cite{Yilmaz1958} --- either import the ghost or leave the standard
field-equation framework. A healthy field-theoretic source for this geometry,
with a regular interior in place of the wormhole throat, has been missing.

In this paper we supply one, and we delimit exactly what it does and does not
establish. Three results are shown. First, the projected phase-pressure
deficit of the RefG scalar sector --- a single higher-sector operator with no
propagating time-kinetic degree of freedom --- sources the exponential
exterior exactly: its stress reproduces the Einstein tensor in all four
diagonal components with one common profile function, a four-component
functional match that no constant rescaling could imitate. Second, the
finite-core load selector that decides between the weak (Solar) branch and the
compact branch is closed into a parameter-free statement: the feedback
exponent is fixed by the volume readout of the asymptotic mass, and the
realizing window becomes $r_s/2<r_c<r_s$ in the core radius, equivalently a
...
resulting geometry is horizonless, regular at the center, geodesically
complete, and its effective source is bounded and continuous at the junction.

On this static branch the observational markers are inherited from the known
exterior and are sharp: the shadow radius exceeds the Schwarzschild value by
$4.6\%$ and the ISCO frequency is $0.931$ of the GR value at the same mass.
Unlike tunable horizonless models that can approach the Kerr limit
arbitrarily closely, these numbers cannot be adjusted: current Sagittarius~A*
...
horizon-scale instruments can falsify the branch outright. We also confront
...
theorem \cite{CunhaBertiHerdeiro2017}, and we compute its location and the
depth of the trapping well, which turns out to be shallow across the whole
realizing window; the nonlinear fate of trapped modes remains open, as it
does for the entire horizonless class \cite{CunhaEtAl2023}.

The scope of the paper is fixed here once. All strong-field statements refer
to the static, spherically symmetric branch; the rotating exterior, the
...
astrophysical object are open problems, collected in the Discussion with the
specific reason each one does not affect the static results. The weak-field
sector of RefG coincides with GR through second post-Newtonian order; those
results are consistency checks, and the only GR-discriminating predictions of
the paper are the compact-branch markers.

The paper is organized as follows. Section~\ref{sec:theory} fixes the field
content, the design requirements, the action, and the branch structure, and
states the tensor-sector and cosmological-background properties.
...
The theory is constructed to satisfy five requirements, each of which maps to
...
are dimensionless, the coefficients below are medium response parameters, and
the density scale is carried by $M_*^4$. For a static spherical solid
configuration $B^{AB}$ has one radial and two tangential eigenvalues, denoted
$\lambda_r$ and $\lambda_t$; these appear throughout the compact branch.

\subsection{Action}
\label{sec:action}

The most general polynomial in the invariants through quadratic order, with
the single phase--solid mixing term, is
\begin{align*}
F_{\rm min}={}&
...
\right]+S_{\rm m}[g_{\mu\nu},\psi],
\end{align*}
where matter couples minimally to the metric alone,
$\mathcal L_{\rm EFT}^{>{\rm min}}$ denotes the higher EFT sector of the same
medium (one operator of which becomes active on the compact branch,
Section~\ref{sec:compact}), and $F_{\rm min}^{(H)}$ is the same polynomial
evaluated on the phase-normalized strain invariants defined next. The active
stress of the scalar sector is obtained by metric variation,
\[
\Theta^{\rm RefG}_{\mu\nu}
...
\quad (T^{\rm m}_{\mu\nu}=0),
\]
and is the source of every exterior considered below.
...
\]
with hatted $\widehat I_n$ formed from $\widehat\lambda_r,\widehat\lambda_t$
as $I_n$ are formed from $\lambda_r,\lambda_t$. No derivative of $H$ appears
in $F_{\rm min}^{(H)}$, so $H$ is auxiliary and its variation gives the
...
$H=h$, where $h$ is the metric phase of the compact exterior; there the
hatted invariants equal one, and because the Solar coefficient family is
...
normalization is not aesthetic: Section~\ref{sec:obstruction} exhibits the
explicit tensor obstruction that forbids the raw-invariant insertion on the
compact branch, and the phase-normalized frame is the resolution of that
obstruction. A global identification of $H$ with the solid determinant,
...
In the pure phase projection, with $Y=-2X$, the theory lies in the minimal
Horndeski class \cite{Horndeski1974} with
\[
...
transverse-traceless perturbations on the homogeneous background used here:
its quadratic TT corrections to both the kinetic and gradient terms vanish,
so the local dispersion is $\omega^2=c^2k^2$ and the tensor mode is massless
on the Minkowski branch. Solid and supersolid media can generate a tensor
mass on cosmological backgrounds \cite{CeloriaComelliPilo2017}; for RefG the
corresponding FLRW mass term is an open item of the cosmological extension,
and does not affect the local statements of this paper.
...
$\rho=-p=c_Y^2/(4c_{Y2})$, i.e.\ an exact $w=-1$ component at the background
...
with cosmological data belong to a separate study. We record it here because
...
self-gravitating dynamics parallels the media of
\cite{CeloriaComelliPilo2017}; its phase projection is minimal Horndeski with
constant $G_4$; the timelike gradient of $\Phi$ plays the role that the
aether vector plays in Einstein-aether theory, with the difference that here
it is the gradient of a shift-symmetric scalar and the solid sector supplies
the spatial frame. The compact exterior connects the theory to the
exponential-metric literature cited in the Introduction, with the source
mechanism replaced: where GR realizations require a phantom scalar
...
branch; in the local spherical weak limit it is subleading, $\Delta_p$ is
absent at leading order, and the exterior source-free equation is
\[
\left(r^2h_{\rm eff}'\right)'=0 .
\]
Gauss normalization of the localized source charge fixes
$-r^2h_{\rm eff}'=GM/c^2$, hence
\[
h_{\rm eff}(r)=\frac{GM}{c^2r},\qquad
\Phi_N=-\frac{GM}{r},\qquad
a_r=-\frac{GM}{r^2}.
\]
Newton's law is the weak spherical limit of the chain
...
\end{align*}
related to the standard isotropic PPN metric by
$\beta_{\rm PPN}=\beta_A+\gamma-1$. The first-order field equations give
$\gamma=1$; at second order the medium-strain solution fixes $a_2=4$
...
\]
and on the Solar-compatible coefficient family $K_{\rm pf}=0$. This is the
local coefficient condition used here for
$\alpha_1=\alpha_2=\alpha_3=0$; the full vector-sector PPN analysis, against
the bounds $|\alpha_1|\lesssim10^{-4}$, $|\alpha_2|\lesssim10^{-7}$
...
\]
so the second-order optical coefficient is $q_{2{\rm PN}}=7/4$. We state
plainly what these numbers mean: $\gamma=\beta=1$, $b_2=3/2$, and
...
consistency check --- Cassini, $|\gamma-1|=(2.1\pm2.3)\times10^{-5}$
\cite{Bertotti2003}, and lunar laser ranging constrain it exactly as they
constrain GR --- and not a discriminating test; the fully coupled
self-consistent exterior ODE and a raw Solar-System likelihood remain
separate steps. The Solar branch imposes the second-order relation
$c_{YI1}=2c_{Y2}$, whose kinetic consequence is handled in
...
Two scale statements close the weak field. The technical background
completion contributes $\Delta_{\rm Solar}\sim\Lambda R_\odot^2\sim
5.3\times10^{-35}$ to the Solar metric, leaving the post-Newtonian matching
unchanged. The projected deficit operator of the compact branch is also
directly small here: relative to the Newtonian curvature scale its local
source ratio is $Ce^{-C}/4\approx1.06\times10^{-6}$ at the Solar surface
($C=r_s/R_\odot$), so the diffuse medium stress dominates the weak-Solar
matching and the two branches do not contaminate each other.

The total localized energy fixes both normalizations used above: the same
$E_0$ is read as the Noether inertial mass and as the far-zone source charge,
...
$K_{\pi\pi}=2c_{Y2}-c_{YI1}=0$, while the phase kinetic term stays healthy,
...
The lift used here is a static-silent dynamic operator. With
...
Section~\ref{sec:weak} is untouched, while in the quadratic action it shifts
only the longitudinal kinetic entry and the mixing entry of the
...
\]
both subluminal (Figure~\ref{fig:stability-window}). The falsification object
is the window itself, not the representative point: a ghost, a complex root,
or a superluminal scalar-longitudinal root rejects the parameter branch. The
statements of this section are local flat-background principal-symbol
results; curved-background hyperbolicity and the constraint-closure analysis
are open problems listed in Section~\ref{sec:discussion}.
...
$(r^2\phi')'=0$ with flat normalization at infinity, $\phi=-r_s/r$, and the
biconformal map $h=-\phi/2$, giving the exterior
\[
...
literature, which uses $m=GM/c^2$ and the form $e^{-2m/r}$, note
$r_s=2m$ throughout. The metric has no finite-radius Killing horizon, matches
Schwarzschild through second order in $m/r$, and its Einstein tensor has the
mixed diagonal profile
\[
G^t{}_t=-D,\qquad
G^r{}_r=+D,\qquad
...
\]
Any source of this exterior must reproduce this specific four-component
pattern: opposite signs in the radial and tangential directions, with one
common profile function $D(r)$. In GR the canonical realization is a
massless ghost scalar, and the radial null energy condition is violated at
every radius \cite{Boonserm2018}. The remainder of this section shows how
the RefG scalar sector supplies this source without a propagating ghost,
which finite-core window realizes it, and how the wormhole throat at
$r=r_s/2$ \cite{Boonserm2018} is excised by a regular core.
...
Section~\ref{sec:branches}. On the compact identity medium configuration the
raw invariants are $Y=w$, $\lambda_r=\lambda_t=1/w$ with $w=e^{r_s/r}$.
Inserting them into $F_{\rm min}$ on the physical Solar coefficient slice
gives the stress
\[
...
$\Theta_F^r{}_r/(M_*^4c_{Y2})=19/4$ at $w=2$. This tensor cannot be absorbed
by any constant retuning of the source: the raw radial and tangential
stresses are equal, while the exponential geometry requires them to have
opposite signs. The raw-invariant insertion is therefore excluded as a
compact source, and the compact branch must be --- and is --- formulated on
the phase-normalized action, where the same polynomial is exactly quiet:
$\widehat Y=\widehat\lambda_r=\widehat\lambda_t=1$, so
$\widehat F_{\rm min}=0$, $\widehat\Theta_F^\mu{}_\nu=0$, and the $\phi^A$
Euler residual vanishes. The order of logic matters: the normalization is
not a device that hides a failure; it is the unique frame of the single
action of Section~\ref{sec:action} in which the demonstrated obstruction is
absent, and its weak-field limit ($H=0$) reproduces every Solar result of
Section~\ref{sec:weak} unchanged.

\subsection{The projected deficit source: realizability}
\label{sec:realizability}

The active compact source is one operator of the higher EFT sector, written
with an independent deficit scalar $H_\Delta$:
\[
...
\]
with $\omega_\Delta$ a dimensionless coefficient set to $1$ by the branch
normalization below; on the weak-Solar branch this channel is unloaded, and
its local source ratio at the Solar surface is $\sim10^{-6}$
(Section~\ref{sec:solar}), so the branches do not mix. On the compact
exterior $H_\Delta=h=r_s/(2r)$ solves its own Euler equation,
$(r^2H_\Delta')'=0$, with zero residual, and the operator's stress is
\[
...
\]
Comparing with the Einstein tensor above, $D=8\pi G\,\Delta_P$ and
\[
G^\mu{}_\nu-8\pi G\,\Theta^\mu{}_\nu=0
\]
in all four diagonal components. This is the central result of the section,
and we state its strength precisely: the match is a four-component
functional identity with one common profile, holding at every radius. A
single fitted constant could rescale one amplitude; it could not produce the
correct relative signs and the common radial profile in all four components
simultaneously. The only quantity fixed by hand is the overall normalization
$\omega_\Delta=1$.

In effective-fluid language the active source has
$\rho_a=-\Delta_P$, $p_{r,a}=-\Delta_P$, $p_{t,a}=+\Delta_P$, so the radial
null energy condition is violated at every exterior radius,
...
The decisive difference from the phantom realization is structural, not
verbal. A ghost scalar carries a propagating degree of freedom with
wrong-sign time kinetic energy. The projector $\gamma^{\mu\nu}$ removes the
time-kinetic direction of $H_\Delta$ on the static branch: the operator
contributes no $\dot H_\Delta^2$ term, its second variation is a positive
static stiffness,
\[
...
\right]>0,
\]
and $H_\Delta$ enters the static problem as an elliptic constraint field, not
as a phantom wave. The propagating scalar spectrum of the theory is carried
by the $F_{\rm min}$ sector analyzed in Section~\ref{sec:stability}. The
null-energy violation of the effective source is thus achieved without a
wrong-sign kinetic term in any propagating sector --- which is precisely
what a healthy realization of this geometry requires. The full perturbation
spectrum around the compact background, including the coupled
$\{g,\Phi,\phi^A,H_\Delta\}$ system, remains the open dynamical problem
recorded in Section~\ref{sec:discussion}.
...
\]
and the equation $Q_0=Qe^{-Q/2}$ alone has two Lambert branches
$Q_\pm=-2W_{0,-1}(-Q_0/2)$, so $Q_0$ by itself does not select the branch.
The selection is made by the source amplitude through the deficit feedback
\[
...
=-\chi q_\star-1<0,
\]
a stable fixed point for any $S,\chi>0$; since $S(Q)=\tfrac{3Q}{2}e^{3\chi
Q/2}$ is monotone, the amplitude selects a unique load.
...
channel. We classify the two facts precisely: the coefficient-independence of
$\chi$ is an action-level statement, while the volume-readout identification
is a physical normalization argument --- the unique exponent consistent with
the extensive (Komar) character of the gravitating charge --- and not an
action-level derivation. With $\chi=1$ the selector is
\[
Q_\star=\frac{2W(S)}{3},
\]
and the requirement that the throat radius $r_s/2$ lie inside the core while
the photon sphere lies outside, $1<Q_\star<2$, becomes the parameter-free
window
\[
\frac32e^{3/2}<S<3e^{3},
\qquad
...
dynamical question is stated openly in Section~\ref{sec:discussion}.

\subsection{The regular core: completeness of the geometry}
\label{sec:core}

The exterior alone is geodesically incomplete, and contains the wormhole
throat at $r=r_s/2$ \cite{Boonserm2018}. Both features are removed by a
...
\end{align*}
match the exterior $\log A_+=q/x$, $\log B_+=-q/x$ at $x=1$ through value,
...
horizonless geometry. The honest limit of this construction is equally
...
normalization ($r_s=2m$). The photon sphere sits at $r_{\rm ph}=r_s$, the
...
\]
a $+4.63\%$ shadow-scale excess over Schwarzschild at the same mass. Massive
circular orbits give the ISCO from $r^2-3r_sr+r_s^2=0$,
\[
...
\cite{MakukovMychelkin2018}. (Incidentally $r_{\rm ISCO}=\varphi^2 r_s$ with
$\varphi$ the golden ratio; this is algebra, not physics.) What the present
construction adds to these known numbers is their setting: they are now
markers of a geodesically complete regular object realized by a healthy
EFT source in a parameter-free compactness window.
...
maximum at $r=r_s$ is the unstable light ring; $V$ is continuous and $C^1$
at the junction with $V'(r_c)>0$ for $q>1$, and rises as the centrifugal
wall $e^{-q}/r^2$ at the regular center; consequently $V$ has exactly one
interior minimum --- a stable light ring inside the core --- realizing the
light-ring pairing required for horizonless ultracompact objects
\cite{CunhaBertiHerdeiro2017}. Its location and the depth of the trapping
...
Table~\ref{tab:observations} collects the confrontation of both branches
with data. The weak-field rows are GR-degenerate by construction; the
...
The shadow prediction deserves its precise statement. The Event Horizon
...
The static branch predicts $\delta=+0.046$, which lies within the
$2\sigma$ bands but about $1$--$1.5\sigma$ above the measured central
values \cite{Vagnozzi2023}. Two caveats accompany any such comparison: the
...
caveats, the position is the strongest feature of the model: because the
window is parameter-free, the prediction cannot retreat toward the
Schwarzschild value, and next-generation horizon-scale measurements can
falsify the branch outright.
...
viability requires a dedicated evolution, not an assertion, and we make
none. In the gravitational-wave channel, the prompt ringdown of the cored
...
WesterweckEtAl2018}; the quasinormal spectrum of the cored geometry is open.

\subsection{Falsifiability and open problems}
\label{sec:open}

What kills the branch, and what kills the framework, are different
statements and we give both. The compact branch is excluded if
horizon-scale shadow measurements of an object independently shown to be in
the realizing window exclude a $+4.6\%$ excess, or if the trapped-mode
instability is shown to destroy the cored configuration on astrophysical
timescales, or if the rotating extension (once constructed) fails the
existing spin-sensitive constraints. The framework as a whole loses its
...
indistinguishable from GR at currently accessible orders --- a falsifiable
position, not a safe one. The scalar-sector stability window of
...
rejects the parameter branch outright.
...
the solid sector; (vii) the explicit vector-sector PPN parameters
...
properties and one sharp prediction. The weak static sector reproduces
Newton's law through a refractive source--index chain and coincides with GR
through second post-Newtonian order; the tensor sector is luminal
identically; the scalar-longitudinal sector has an open, healthy stability
...
parameter-free; and a $C^2$-matched core renders the geometry horizonless,
regular, and geodesically complete, while excising the wormhole throat of
the bare exterior. The branch inherits the known geodesic numbers of the
exponential metric and turns them into falsifiable predictions of a
complete geometry: a $+4.6\%$ shadow excess, already under mild tension
from Sagittarius~A* and decidable by next-generation instruments, and an
ISCO frequency $0.931$ of the GR value. The rotating exterior, the
...
No observational or experimental datasets were generated or analysed in
...
archived at
% TODO before submission: upload RG/work gates to Zenodo/GitHub and insert
% the DOI/URL on the next line, then delete this comment.
\texttt{[repository DOI]}.
