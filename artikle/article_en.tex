...
The physical mechanism is the following. Matter creates a phase-pressure deficit in the base medium; the active stress obtained from the action is collected, in the weak static regime, into the variable $h_{\rm eff}$; the minimal metric coupling of matter and the static optical limit define the refractive index $n_{\rm eff}=\exp(h_{\rm eff})$; and the acceleration of test matter is determined by the gradient of this index. In the local spherical channel the Bernoulli response gives Newton's law, while in the coherent vortical spherical limit the same source equation describes the deep-MOND limit and the algebraic core of the baryonic Tully--Fisher relation (BTFR).

The mathematical results obtained in the paper are as follows. The quadratic Bernoulli pressure and the linear Tolman--Oppenheimer--Volkoff (TOV) source are unified through the response density $\Pi_B'$; the branch $C_6=Y+2I_1-I_3=6$ provides a background $\Lambda$CDM-type balance with the background-normalization parameter $\rho_0$; the weak-field Solar-System branch preserves $\gamma=\beta=1$, $\alpha_i=0$, and the GR-compatible coefficient $q_{\rm 2PN}=7/4$; in the tensor sector the gravitational-wave speed is equal to the speed of light, $c_g=c$; and the extra linear sources vanish for the cosmic microwave background (CMB) test with identical input conditions. In the leading compact phase-spherical strong-field branch, $F_{\rm min}$ is the minimal medium core, while the source of the exponential exterior is the projected Bernoulli stress $\mathcal L_B^\perp$. Together with this source, the exterior phase equation and the biconformal operational map give the exponential exterior. The geodesic analysis of this exterior gives $r_{\rm ph}=r_s$, $b_c=e r_s$, $r_{\rm ISCO}=\varphi^2 r_s$, and $f_{\rm ISCO}=0.931\,f_{\rm ISCO}^{\rm GR}$, where $\varphi=(1+\sqrt5)/2$. This is the central falsifiable static marker of the paper: the static shadow scale shifts by $+4.63\%$ relative to the Schwarzschild value in GR.

The strong-field phase branch is distinct from the weak physical Solar-System exterior: near the Sun, diffuse medium stress remains in the 2PN balance and gives $q_{\rm 2PN}=7/4$; outside an ultradense compact source, the phase deficit dominates and its projected Bernoulli stress balances the exponential branch. The next data layer includes the $H(z)/w(z)$ channel, SPARC/RAR and cluster comparison, the active CMB problem without particle dark matter, additional gravitational-wave polarizations, and EHT/ngEHT/BHEX tests of the rotating strong-field sector.
...
The complete physical picture of the theory includes particles, quantum regimes, strong fields, galactic morphology, and cosmological dynamics. This paper presents the main axis that directly expresses the gravitational mechanism: the refractive source, its Newtonian limit, the spherical algebraic core of the galactic weak-field sector, and the required consistency conditions with observational data.

The structural logic of the paper is the following. A localized oscillon in the base medium leaves a phase-pressure tail; this tail generates active stress; in the weak-field limit the active stress is collected into the variable $h_{\rm eff}$; $h_{\rm eff}$ is physically interpreted as the refractive index $n_{\rm eff}$, whose gradient determines the motion of an object. Near a localized source this mechanism gives the Newtonian profile, in the coherent spherical galactic limit it gives the algebraic MOND/BTFR core, and in cosmological averaging it gives a background balance.
...
\node[figbox] (mond) at (2,-1.55) {Spherical vortex\\MOND/BTFR\\core};
...
This formulation sharply separates the minimal dynamical core from the branch-fixing additions. The function $F_{\rm min}$ is the minimal low-energy polynomial basis in the invariants $Y$, $I_1$, $I_2$, and $I_3$: it contains the phase term, the spatial-density term, the shear/volume response, and the $YI_1$ coupling. Higher-order operators form the EFT layer built on this core. The term $\mathcal L_{\rm branch}$ denotes the source term carried by a specific physical branch: it vanishes in the weak-field minimal core, while in the phase-dominated compact branch it is the projected Bernoulli stress of the medium, $\mathcal L_B^\perp$. Its static form is used in Sec.~\ref{sec:strong-field} as the source of the exponential exterior. The conditions $C_6=6$ and $Z$ are then used to select the background and stability channel in which cosmological balance, local stability, and Solar-System compatibility are tested simultaneously.
...
The factor $2\pi$ denotes the normalization of the phase cycle. With this normalization, the baryonic channel, the vortical refractive channel, and the total acceleration form one algebraic system. In this spherical working limit, the cosmological phase scale enters the MOND transition law.
...
The status of this result is precise: $\mu=x/(1+x)$ and the BTFR are closed within the two-channel vortical construction. This is the spherical algebraic core. A full RefG-dynamical derivation, the selection of the $2\pi$ normalization, the complete disk-field solution, and comparison with SPARC/RAR data belong to the next layer.
...
The cosmological problem therefore splits into two parts. The first is the background balance fixed by $C_6=6$; it remains compatible with the Solar-System 1PN regime and carries the normalization parameter $\rho_0$. The second is slow evolution between cosmological epochs, tested through the observables $H(z)$ and $w(z)$. In the present section, the first part is described through the invariant surface $C_6=6$.
...
The quadratic term fixes the surface $C_6=6$ as a minimum of the action. The parameter $\rho_0$ is expressed in the same energy-density units as the Lagrangian density of the effective field theory sector. If the full sector is written as $M_*^4F$, then $\lambda_6$ may be dimensionless, while the common factor $M_*^4$ carries the physical density scale. In this notation, $\rho_0$ is the background density scale, not a microphysically calculated number in this section.
...
\end{equation}
In this paper $\rho_0$ is a background parameter; the microphysical scale of $\rho_0$ is a separate cosmological-microphysical problem.
...
\end{align}
The $C_6=6$ branch describes a $\Lambda$CDM-type background balance with the normalization parameter $\rho_0$; the dynamical clock describes slow evolution between epochs and provides a channel for comparison with DESI-type data. The full empirical comparison of this channel enters the next data analysis.

This section establishes the background algebraic balance $C_6=6$: a $\Lambda$CDM-type background, the normalization parameter $\rho_0$, the vanishing of the phase current added by the completion on the $C_6=6$ surface, stable kinetic support through $Z$, and Solar-System 1PN compatibility. In this core, $\rho_0$ carries the normalization of the background density; its microphysical calculation is a separate problem. Attractor dynamics and the empirical tests with $H(z)$, $w(z)$, BBN, CMB, BAO, and SNe belong to the data layer.
...
In the 2PN sector, the physical ordering is fixed by the regime. The weak-field Solar-System exterior, a restricted prescribed form, and the independent strong-field map are different objects. In this text, $q_{\rm 2PN}$ denotes the dimensionless coefficient of the 2PN optical/deflection expansion in the normalization in which the weak Schwarzschild exterior gives $7/4$. In the restricted isotropic zero-stress form, the medium stress is set to zero, and under this condition one obtains $q_{\rm 2PN}=10$. In the physical Solar problem, the medium stress enters the source side of the equation $G=\kappa T$, and the radial deformation participates in the balance of the exterior solution. In this case, the old angular remnant becomes a differential equation for the radial deformation. On the physical Solar branch,
\begin{equation}
q_{\rm 2PN}=\frac74 .
\end{equation}
...
\node[regimebox, above right=of core] (galaxy) {Spherical vortex core\\$\mu=x/(1+x)$\\BTFR};
\node[regimebox, below left=of core] (cosmo) {Homogeneous background\\$C_6=6$\\$\rho_0$ normalization};
...
For the cosmic microwave background (CMB), the key result in this section is the compatibility test with fixed identical input conditions. The additional phase-spatial sector of RefG is tested on the same background and perturbative inputs used by the standard CMB calculation.
...
Here ``identical input conditions'' means the same background history $H(a)$, the same matter content, the same recombination, and the same primordial spectrum. The CDM component present in the comparison LCDM model remains the same input in this identity test. This is the CMB compatibility identity for fixed input conditions; replacing particle dark matter is a separate active-Boltzmann problem.

The closed result of this section is the CMB identity for identical input conditions: the additional phase-spatial sector of RefG leaves the standard CMB calculation unchanged when the background history, matter content, recombination, and primordial spectrum are unchanged. Constructing the CMB without particle dark matter is a separate active-source layer. It requires the source density and velocity, the Euler equation, the closure of the shear response and metric slip, and full comparison with Planck/BAO/lensing data.
...
In the standard formalism, this fact is postulated through the equivalence principle: in free fall, mass cancels out of the equation of motion. In the leading Noether channel of RefG, both quantities are tied to the same localized energy. Inertia is the response of a localized structure to acceleration, while gravitational charge is the long-range refractive source of the same structure.
...
\end{equation}
and the low-speed force coefficient is $E_0/c^2$. In this RefG channel, inertia and exterior refractive charge are tied to the same localized energy. The inertial mass is defined from the Noether four-momentum as
\begin{equation}
...
Thus the mass cancels from the equation of free fall. Radiation and delayed effective dressing contributions remain as higher-order corrections to the response; the leading coefficient relating force and acceleration, $F=Ma$, is fixed by the localized rest energy.
...
The result connects to the refractive model in the following form: the effective exterior mass is the asymptotic charge of localized energy that carries the refractive deficit of the base medium. The leading identity between inertial mass and exterior refractive charge is fixed here; the full microphysical evolution of the oscillon belongs to the particle/structural program.
...
In the leading phase-spherical strong-field branch, outside the source the phase potential satisfies
\begin{equation}
...
\phi=-\frac{r_s}{r}.
\end{equation}
On this branch, the biconformal operational map is defined by
\begin{equation}
h=-\frac{\phi}{2},\qquad
g_{tt}\propto \exp(-2h),\qquad
g_{\rm spatial}\propto \exp(2h).
\end{equation}
Thus $h=r_s/(2r)$, $AB=1$, $n=\exp(h)$, and the weak-field limit has $\gamma=1$. This map sends the phase solution to the exponential exterior
\begin{equation}
...
In the compact branch, the gradient of the phase-pressure deficit enters as a source through the projected Bernoulli stress of the spatial medium. In the static spherical exterior,
\begin{align}
Z_\perp &= \frac{(h')^2}{A}
        = \frac{r_s^2\exp(-r_s/r)}{4r^4}, \\
\mathcal L_B^\perp &= \frac{Z_\perp}{8\pi G}, \\
\Delta_P &= \frac{Z_\perp}{8\pi G}
          = \frac{r_s^2\exp(-r_s/r)}{32\pi G r^4}.
\end{align}
The mixed components of this source are
\begin{equation}
\Theta^t{}_t=-\Delta_P,\qquad
\Theta^r{}_r=\Delta_P,\qquad
\Theta^\theta{}_\theta=\Theta^\phi{}_\phi=-\Delta_P .
\end{equation}
The mixed Einstein profile of the exponential exterior is
\begin{equation}
G^t{}_t=-D,\qquad
G^r{}_r=D,\qquad
G^\theta{}_\theta=G^\phi{}_\phi=-D,\qquad
D=8\pi G\Delta_P .
\end{equation}
Therefore every diagonal component satisfies
\begin{equation}
G^\mu{}_\nu-8\pi G\Theta^\mu{}_\nu=0 .
\end{equation}
The static exponential exterior is therefore read as the solution of the compact phase branch of RefG.

The exponential-metric class is described in the literature as a traversable-wormhole geometry \cite{Boonserm2018}. In RefG the source of the same static geometry is the active medium stress $\Theta^{\rm RefG}$ given above. The standard effective-fluid dictionary reads this profile as a background-subtracted active deficit,
\begin{equation}
\rho_a=-\Delta_P,\qquad p_{r,a}=-\Delta_P,\qquad p_{t,a}=\Delta_P .
\end{equation}
This is the contrast that sources the geometry. The full physical RefG medium includes a positive homogeneous background load $\rho_\ast,p_\ast$:
\begin{equation}
\rho_{\rm tot}=\rho_\ast-\Delta_P,\qquad
p_{r,{\rm tot}}=p_\ast-\Delta_P,\qquad
p_{t,{\rm tot}}=p_\ast+\Delta_P .
\end{equation}
Therefore the radial null energy condition (NEC) sum of the full medium is
\begin{equation}
\rho_{\rm tot}+p_{r,{\rm tot}}=\rho_\ast+p_\ast-2\Delta_P .
\end{equation}
The deficit profile
\begin{equation}
\Delta_P=\frac{r_s^2\exp(-r_s/r)}{32\pi G r^4}
\end{equation}
has its maximum at $r=r_s/4$:
\begin{equation}
\Delta_P^{\max}=\frac{8e^{-4}}{\pi G r_s^2}.
\end{equation}
Thus the full-medium condition of the strong phase branch is the finite inequality
\begin{equation}
\rho_\ast+p_\ast\ge 2\Delta_P^{\max}
=\frac{16e^{-4}}{\pi G r_s^2}.
\end{equation}
Under this condition the full base medium keeps a positive radial NEC, while $\Theta^{\rm RefG}$ remains the active source of the phase-pressure deficit that creates the exponential exterior. The corresponding positive-density condition is $\rho_\ast\ge\Delta_P^{\max}$. The geodesic analysis of this exterior gives the photon and massive-orbit markers below.

On the same static compact branch, $\mathcal L_B^\perp$ does not create an independent negative time-kinetic scalar mode: the projector leaves a spatial stiffness source,
\begin{equation}
\delta^2\mathcal L_B^\perp
=\frac{\exp(-r_s/r)}{8\pi G}
\left[
(\delta h_r)^2+\frac{(\delta h_\theta)^2}{r^2}
+\frac{(\delta h_\phi)^2}{r^2\sin^2\theta}
\right],
\end{equation}
which is positive for $G>0$ and $r>0$. The principal symbol of this source is static elliptic/constraint-like; the hyperbolicity of propagating modes belongs to the full $F_{\rm min}$/p01 medium sector, where the local no-ghost and mixed-mode stability window is nonempty.

In this exterior no finite-radius horizon appears. Its strong-field test is therefore directly tied to tests of horizonless ultracompact objects. The astrophysical use of this static marker proceeds through dynamical matching to the inner core, a rotating exterior, the QNM/echo channel, and comparison with EHT/ngEHT/BHEX-type ray tracing.

The physical criterion for selecting the branch is the compactness of the source, the exterior matching condition, and the weight of diffuse medium stress. Near an extended weak-field body such as the Sun, stress and radial deformation participate in the 2PN exterior balance and give $q_{\rm 2PN}=7/4$. Outside an ultradense compact source, the static phase deficit dominates; therefore $b_c=e r_s$ and $r_{\rm ISCO}=\varphi^2 r_s$ belong to this compact phase branch.

For null rays, the radial barrier is
\begin{equation}
B_{\rm ph}(r)=\frac{\exp(-2r_s/r)}{r^2}.
\end{equation}
This barrier is the ratio of the temporal and angular factors, $A/C$: in the exponential exterior $A=\exp(-r_s/r)$, while the angular factor is $C=\exp(r_s/r)r^2$, hence $A/C=\exp(-2r_s/r)/r^2$.
...
\end{equation}
or a $+4.63\%$ scale shift of the shadow.
...
=\varphi^2r_s .
\end{equation}
Here $\varphi=(1+\sqrt5)/2$ is the golden-ratio number.
...
The status of this result is clear. The static spherical phase branch is closed at the level of the phase equation, the biconformal map, the projected Bernoulli source, the full-medium NEC condition, the minimal static stability gate, and the geodesic analysis. The source of the compact exterior is $\mathcal L_B^\perp$; $F_{\rm min}$ remains the minimal medium core and does not by itself carry this exponential source. The remaining layers are the full dynamical matching to a finite core, the rotating strong-field sector, the QNM/echo spectrum, and observational ray tracing.

The observational test of this static result moves to a rotating exterior, accretion and plasma models, ray tracing, QNM/echo analysis, and comparison with EHT/ngEHT/BHEX data. Here the strong-field sector enters as the static spherical result obtained from the phase equation and projected Bernoulli source, and as the starting point of the subsequent observational program.
...
Observational separation of this marker requires percent-level shadow-scale accuracy, a rotating compact-source model, accretion/plasma control, and EHT/ngEHT/BHEX-class ray-tracing comparison. Cosmological dynamics, MOND/clusters, and the CMB without particle dark matter are the next data-test channels.
...
The working phase-cycle normalization fixes $a_0=cH_0/(2\pi)$; within the two-channel vortical construction one obtains $\mu=x/(1+x)$ and the BTFR. &
...
$C_6=6$ gives an exact background $\Lambda$CDM-type algebraic balance with the normalization parameter $\rho_0$; the redshift channel requires compatibility with the CMB blackbody spectrum, SNe $(1+z)$ time dilation, and the Tolman $(1+z)^4$ surface-brightness law. &
Microphysical calculation of $\rho_0$, attractor dynamics, $H(z)$, $w(z)$, and comparison with BBN/CMB/BAO/SNe data. \\
\bottomrule
\end{tabularx}
\end{table}
...
On the branch with fixed identical input conditions, the additional linear sources vanish and $C_\ell^{\rm RefG}=C_\ell^{\rm LCDM}$; the CDM component of the comparison LCDM model remains the same input. &
...
The projected Bernoulli source balances the static exterior; $\Delta_P^{\max}=8e^{-4}/(\pi G r_s^2)$ and the full-medium radial NEC condition is $\rho_\ast+p_\ast\ge16e^{-4}/(\pi G r_s^2)$; $\mathcal L_B^\perp$ carries no standalone phantom scalar and has positive static stiffness; $r_{\rm ph}=r_s$, $b_c=e r_s$, $r_{\rm ISCO}=\varphi^2r_s$, $f_{\rm ISCO}=0.931\,f_{\rm ISCO}^{\rm GR}$; the static shadow scale shifts by $+4.63\%$ relative to the Schwarzschild GR value. &
Finite-core dynamical matching, nonlinear strong-field terms, rotating exterior, ray tracing, accretion/plasma model, QNM/echo, and EHT/ngEHT/BHEX comparison. \\
...
RefG is a phase-spatial framework of effective field theory (EFT) in which the source of gravity is interpreted as a refractive deficit of the base medium. The weak-field Solar-System branch is compatible with GR, while the difference lies in the mechanism itself: Newtonian force, the spherical algebraic MOND/BTFR core, the leading Noether channel of inertia, and the cosmological background balance are written in the language of one phase-pressure response.

The central line of the paper is the following. The minimal action and the active-stress convention lead to the weak static refractive source-index chain; this chain is compatible with the local stability window, the Solar-System 1PN/2PN limit, the tensor result $c_g=c$, the CMB test with identical input conditions, and the Noether channel of inertia.

The static strong-field axis stands on the leading compact phase-spherical branch. In this branch $F_{\rm min}$ is the minimal medium core, while the source of the exponential exterior is the projected Bernoulli stress $\mathcal L_B^\perp$; together with this source, the phase equation and the biconformal map give the exponential exterior. On this exterior the photon barrier gives $b_c=e r_s$, while the marginal stability of massive orbits gives $r_{\rm ISCO}=\varphi^2r_s$ and $f_{\rm ISCO}=0.931\,f_{\rm ISCO}^{\rm GR}$. The static shadow scale changes by $+4.63\%$ relative to the Schwarzschild GR value. The Solar-System result $q_{\rm 2PN}=7/4$ remains in the weak physical exterior branch; the strong-field phase value $q_{\rm 2PN}=2$ is a separate strong-field branch. Finite-core dynamical matching, the rotating strong-field sector, and observational ray tracing remain subsequent work.
