...
The exponential-metric class is described in the literature as a traversable-wormhole geometry \cite{Boonserm2018}. In RefG the source of the same static geometry is the active medium stress $\Theta^{\rm RefG}$ given above. In the standard effective-fluid dictionary, this profile corresponds to
\begin{equation}
\rho=-\Delta_P,\qquad p_r=-\Delta_P,\qquad p_t=\Delta_P ,
\end{equation}
so the radial NEC is violated, while the tangential NEC is zero. The physical source in RefG is not an ordinary matter fluid; it is the projected active stress of the phase medium. The geodesic analysis of this exterior gives the photon and massive-orbit markers below.
...
The status of this result is clear. The static spherical phase branch is closed at the level of the phase equation, the biconformal map, the projected Bernoulli source, and the geodesic analysis. The source of the compact exterior is $\mathcal L_B^\perp$; $F_{\rm min}$ remains the minimal medium core and does not by itself carry this exponential source. The remaining layers are the full dynamical matching to a finite core, the rotating strong-field sector, and observational ray tracing.
...
The projected Bernoulli source balances the static exterior; $r_{\rm ph}=r_s$, $b_c=e r_s$, $r_{\rm ISCO}=\varphi^2r_s$, $f_{\rm ISCO}=0.931\,f_{\rm ISCO}^{\rm GR}$; the static shadow scale shifts by $+4.63\%$ relative to the Schwarzschild GR value. &
