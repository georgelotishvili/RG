...
\author{George Lotishvili$^{a,*}$ \href{https://orcid.org/0009-0009-6788-713X}{(ORCID 0009-0009-6788-713X)}\\[0.5em]
...
\begin{abstract}
Refractive Gravity (RefG) is a phase-spatial effective field theory with an Einstein--Hilbert metric core and a medium sector built from one phase scalar $\Phi$ and three spatial scalars $\phi^A$. The paper gives a self-contained gravitational calculation chain: the active medium stress is fixed by metric variation, the weak static Bianchi/Tolman--Oppenheimer--Volkoff source equation and exterior Gauss normalization give the refractive profile, and minimal metric coupling gives Newton's law for slow test bodies. The Solar branch gives $\gamma=\beta=1$ and $q_{2{\rm PN}}=7/4$; the displayed preferred-frame source combination vanishes, and the principal tensor speed has $c_g=c$. The compact phase branch is a different exterior of the same normalized-invariant EFT sector. Its projected phase-pressure deficit source gives a horizonless exponential exterior with photon sphere $r_{\rm ph}=r_s$, critical impact parameter $b_c=e\,r_s$, innermost stable circular orbit radius $r_{\rm ISCO}=\varphi^2r_s$, and orbital frequency $0.931$ times the Schwarzschild value. Direct black-hole-imaging comparison requires the rotating exterior, finite-core matching, and ray-traced plasma/accretion modeling.
\end{abstract}

\section{Introduction}

General Relativity (GR) supplies the metric core of gravitational dynamics. RefG keeps this core and adds a phase-spatial medium sector whose action invariants vary to an active stress tensor. In the weak static limit that stress is read as a refractive profile.

The name ``Refractive Gravity'' refers to this source mechanism: the gravitational response follows from a phase-pressure deficit that acts refractively on matter and light. Matter remains minimally coupled to the metric; the effective-medium difference is that the active stress gives a source-index chain that produces the Newtonian profile near a localized weak source and a definite compact-branch prediction in the strong static regime.
...
The compact static branch uses an exponential exterior. This metric form has an independent literature, including its historical association with Yilmaz's gravitational theory \cite{Yilmaz1958} and its analysis as a traversable-wormhole geometry \cite{Boonserm2018}; strong-field alternatives to GR provide the comparison setting \cite{Damour1992,Damour1993,Damour1996}. In RefG the exterior is the phase-spherical compact branch sourced by the projected phase-pressure deficit stress of the medium.

This paper presents the self-contained gravitational chain that can be checked internally: action, active stress, refractive source, Newtonian limit, local principal-symbol stability, Solar-System compatibility, tensor speed, inertial identity, and the compact static predictions.
...
\item \textbf{Base medium.} The phase-spatial EFT sector described by $\Phi$ and $\phi^A$.
\item \textbf{Active stress.} The metric variation of the medium action, $\Theta_{\mu\nu}^{\rm RefG}$.
\item \textbf{Refractive source.} The active-stress channel that fixes $h_{\rm eff}$ and $n_{\rm eff}$.
\item \textbf{Source-index chain.} The calculation $\Pi_\Delta'\to h_{\rm eff}\to n_{\rm eff}\to a_r$.
\item \textbf{Phase-pressure deficit.} The projected pressure channel $\Delta_P=Z_\Delta^\perp/(8\pi G)$ that supplies the compact exterior source.
\item \textbf{Phase-normalized strain invariants.} The $H$-rescaled variables $\widehat Y$, $\widehat\lambda_r$, and $\widehat\lambda_t$ used in a branch frame.
\item \textbf{Biconformal map.} The compact metric rule using paired factors $e^{-2h}$ and $e^{2h}$.
\item \textbf{Branch.} A background regime of the same normalized-invariant EFT sector, selected by compactness and boundary matching.
\item \textbf{Weak-Solar branch.} The unloaded branch with $H=0$ and Solar $1$PN/$2$PN GR-compatible coefficients.
\item \textbf{Compact phase-spherical branch.} The loaded branch with $H=H_\Delta=h$ that gives the exponential exterior.
\item \textbf{Dynamic completion.} The $\epsilon_B,\epsilon_M$ kinetic extension opening the scalar-longitudinal subluminal window.
\item \textbf{ESS operator.} The static-silent effective-solid-sector operator used for Solar kinetic completion.
\item \textbf{Raw-invariant diagnostic.} The unnormalized compact-invariant audit; it is not the compact action.
\end{list}
...
branch this channel is small. In the compact $C^2$ matching branch
...
absolute-invariant compact insertion audited below is a diagnostic trial, not a
...
G_5=0 .
\]
Because $G_4$ is constant and $G_5=0$, the principal tensor speed is luminal.
...
\]
with the full vector-sector PN audit belonging to the complete PPN extension
...
c_g=c.
\]
The spatial medium completion leaves this principal result unchanged on the
homogeneous background used here. For transverse-traceless perturbations,
\[
\delta K_{\rm TT}=0,\qquad
\delta G_{\rm TT}=0,
\]
so the displayed core has no tensor masslike or anisotropic-stress correction
at this order, and the principal dispersion relation is
\[
\omega^2=c^2k^2.
\]
Thus the tensor wave speed is luminal in this RefG core.
...
The compact-branch static consistency chain is then: $(r^2\phi')'=0$ fixes $\phi=-r_s/r$; the biconformal map gives $h=r_s/(2r)$ and the exponential metric; the independent deficit field has $H_\Delta=h$ and zero harmonic residual; $\mathcal L_\Delta^\perp=Z_\Delta^\perp/(8\pi G)$ gives the active source written above; with this source the diagonal field-equation residuals vanish and $p_t-p_r=2\Delta_P$. This is the conditional static exterior equation of the compact branch. The remaining finite-core question is dynamical branch selection: which compact interiors reach this boundary regime.
...
\]
The marginal stability condition $dL_{\rm circ}^2/dr=0$ gives
\[
r^2-3r_sr+r_s^2=0.
\]
Therefore
...
\]
Using the Schwarzschild normalization
$(\Omega_{\rm ISCO}^{\rm GR})^2=c^2/(54r_s^2)$ at the same mass, at the ISCO
\[
...
Schwarzschild comparison; the circular-orbit condition
$dL_{\rm circ}^2/dr=0$ gives
...
\]
This is the conditional static result of the compact phase branch. Its direct comparison with EHT-class images belongs to the rotating exterior and ray-tracing problem.
...
No external datasets were generated or analysed. All calculations needed to
verify the results are included in the article.
