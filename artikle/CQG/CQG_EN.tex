...
\item \textbf{Branch.} A background regime of the same normalized-invariant EFT sector, selected by the finite-core load and source amplitude.
...
load $Q=r_s/r_c$ and the input compactness $Q_0$.  Surface-clock matching gives
\[
q_v=\frac{3Q}{2},\qquad Q_0=Qe^{-Q/2}.
\]
...
\]
Thus the finite-core source amplitude $S$ selects a unique branch.  The high
compact branch is selected for
\[
S>S_{\rm fold}=3e^{3\chi},
\]
while object-specific values of $S$ and $\chi$ belong to the finite-core
equation of state or collapse history.
...
Fourth, the compact selector must put the finite core on the high branch:
\[
Q_\star=\frac{2W(\chi S)}{3\chi},\qquad
S>S_{\rm fold}=3e^{3\chi}.
\]
If this condition fails, the compact strong-field branch is not occupied.
When it holds, the compact phase branch gives definite static branch predictions:
\[
r_{\rm ph}=r_s,\qquad
b_c=e\,r_s,\qquad
...
Q_\star=\frac{2W(\chi S)}{3\chi},
\]
with the compact branch selected for $S>3e^{3\chi}$. Direct comparison with
