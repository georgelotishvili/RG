...
The compact static branch uses an exponential exterior. Exponential metric geometries have an independent literature as exact geometries, including their historical association with Yilmaz's gravitational theory \cite{Yilmaz1958}; the same metric form has also been analyzed as a traversable-wormhole geometry \cite{Boonserm2018}. RefG uses this exterior in a different role: it is the phase-spherical compact branch sourced by the projected phase-pressure deficit stress of the medium.
...
regimes of the independent deficit channel $H$: the weak Solar guard is the
unloaded background $H=0$, while the compact pure-phase exterior is the loaded
background $H=h$. The action is not changed between branches.
...
The $H$ Euler source also vanishes on the weak Solar guard. Write
\[
...
through the Solar guard order, not only a formal reduction of the action.
...
The same exponential metric form is historically associated with Yilmaz's gravitational theory \cite{Yilmaz1958} and also appears in the geometric literature as a traversable wormhole geometry \cite{Boonserm2018}. In RefG the metric is not introduced by adopting either source model. It is obtained from the compact phase-spherical branch: the active projected deficit stress of the phase-spatial medium supplies the exterior source.
