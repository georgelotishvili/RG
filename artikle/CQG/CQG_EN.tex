...
\item \textbf{Weak-Solar branch.} The unloaded $H=0$ branch with $\gamma=\beta=1$, vanishing preferred-frame source, and $q_{2{\rm PN}}=7/4$.
\item \textbf{Compact phase-spherical branch.} The loaded $H=H_\Delta=h$ branch in which the reduced phase equation and projected deficit source give the exponential exterior.
\item \textbf{Dynamic completion.} The static-silent $\epsilon_B,\epsilon_M$ extension that changes the scalar-longitudinal kinetic/mixing entries and opens the subluminal window.
...
\item \textbf{Raw-invariant diagnostic.} The audit that evaluates the compact exterior with unnormalized raw invariants; it is a diagnostic obstruction, not the compact action.
\end{list}
