...
\begin{abstract}
Refractive Gravity (RefG) is a phase-spatial effective field theory with an Einstein--Hilbert metric core and a medium sector built from one phase scalar $\Phi$ and three spatial scalars $\phi^A$. The paper gives a self-contained gravitational calculation chain: metric variation defines the active medium stress, the weak static Bianchi/Tolman--Oppenheimer--Volkoff source equation fixes the refractive profile, and minimal metric coupling gives Newton's law for slow test bodies. The Solar branch gives $\gamma=\beta=1$, $\alpha_1=\alpha_2=\alpha_3=0$, $q_{2{\rm PN}}=7/4$, and the tensor sector has $c_g=c$. The compact phase branch is a different exterior: the raw absolute-invariant compact insertion of $F_{\rm min}$ is rejected, while the phase-normalized $F_{\rm min}$ sector has zero density, zero metric stress, and zero $\phi^A$ Euler residual on the pure-phase exterior. The projected phase-pressure deficit source then gives a horizonless exponential exterior with photon sphere $r_{\rm ph}=r_s$, critical impact parameter $b_c=e\,r_s$, innermost stable circular orbit radius $r_{\rm ISCO}=\varphi^2r_s$, and orbital frequency $0.931$ times the Schwarzschild value. Direct black-hole-imaging comparison requires the rotating exterior, finite-core matching, and ray-traced plasma/accretion modeling.
\end{abstract}

\section{Introduction}

General Relativity (GR) gives the metric core of gravitational dynamics. RefG keeps this core and adds a phase-spatial medium sector. The local state of the medium is described by action invariants, and its variation produces an active stress tensor. In the weak static limit that stress appears to an external observer as a refractive profile.
...
The construction is an effective field theory. It is written with the usual metric variables and with scalar fields that describe the internal state of the medium. The phase field $\Phi$ tracks the temporal phase-pressure degree of freedom, while the three fields $\phi^A$ track the spatial body of the medium. Their invariants define the allowed local energy functional. This follows the same conservative logic used in scalar-tensor and gravitational EFT work \cite{Horndeski1974,Gubitosi2013,BelliniSawicki2014}, with a different source mechanism: the operative weak-field quantity is the refractive index produced by the active stress.

The tensor sector is restricted by the observed equality of the gravitational-wave and light speeds \cite{Abbott2017GW,Abbott2017GRB}. The RefG tensor bridge therefore uses the Horndeski-compatible constant-$G_4$ sector, so that the tensor speed is exactly luminal. The spatial-medium part is related in spirit to solid and framid EFTs \cite{Endlich2013,Nicolis2015}; in RefG it serves as a gravitational source channel.

The compact static branch uses an exponential exterior. Exponential metric geometries have an independent literature as exact geometries, including their historical association with Yilmaz's gravitational theory \cite{Yilmaz1958}; the same metric form has also been analyzed as a traversable-wormhole geometry \cite{Boonserm2018}. In RefG this exterior is the phase-spherical compact branch sourced by the projected phase-pressure deficit stress of the medium.

This paper presents the self-contained gravitational core that can be checked internally: action, active stress, refractive source, Newtonian limit, local principal-symbol stability, Solar-System compatibility, tensor speed, inertial identity, and the compact static predictions. The result is a single calculation chain from the action to the static compact-branch numbers.
...
One base medium supports distinct mechanical responses. The phase, pressure, and spatial deformation channels used in this paper are separate responses of the same substrate, linked by internal medium interactions. A localized energy concentration creates a deficit in the phase pressure of the base medium. Test matter follows the gradient of the resulting refractive index. In the weak spherical regime this index gradient gives Newton's force; in a compact phase-dominated exterior the same logic leads to the exponential static branch.

At the effective level, a localized matter source is represented by a closed phase-energy state of the medium. The \emph{active stress} is the stress tensor obtained by varying the medium action with respect to the metric. The scalar $h_{\rm eff}$ collects the weak static refractive potential for slow test matter. The slow-body source index is
\[
...
\]
The biconformal map is the compact-branch relation that turns the phase solution into the exponential exterior metric through the paired time/spatial conformal factors $e^{-2h}$ and $e^{2h}$.

Branch selection is controlled by compactness and boundary matching: weak sources keep the diffuse Solar stress contribution, while C2-matched ultradense sources select the phase-pressure-deficit exterior.
...
branch this channel is small. In the compact C2 matching branch
$H_\Delta$ carries the measured source law through
...
$H=0$.
The same covariant EFT action is used in both regimes. Branches are background
regimes of the independent deficit channel $H$: the weak Solar branch is the
unloaded background $H=0$, while the compact pure-phase exterior is the loaded
background $H=h$. The same action covers both branches.
...
unchanged.  Only the dynamic condition that produced the old $p_0=p_2$ locking is
changed.
...
condition is repaired by an independent dynamic channel.  The old $F_{\rm min}+C_6/Z$
...
root is confined to the pre-repair diagnostic. At the flat principal-symbol level
the repaired sector is nondegenerate; curved-background hyperbolicity remains a
...
The Solar-System branch must reproduce the standard post-Newtonian (PN) tests \cite{Will1993,Will2014}. The spherical calculation is first written in a Schwarzschild-like areal-radius working coordinate. With $u=GM/(c^2R)$, the areal time coefficient is denoted by $\beta_A$:
\begin{align}
...
q_{2{\rm PN}}=\frac{7}{4}.
\]
Restricted zero-stress and dark-energy-scale completion tests diagnose branch separation. The compact phase branch is a separate strong-field exterior; its observational signatures are the shadow and innermost stable circular orbit (ISCO) quantities derived below.
...
\]
Thus $q_{2{\rm PN}}=7/4$. Cassini and lunar-laser-ranging tests probe this weak-Solar metric branch; RefG changes the source mechanism, while the weak-Solar $1$PN/$2$PN optical coefficients remain GR-compatible.
...
c_g=c.
\]
The spatial medium completion leaves this result unchanged. For transverse-traceless perturbations,
\[
...
The same exponential metric form is historically associated with Yilmaz's gravitational theory \cite{Yilmaz1958} and also appears in the geometric literature as a traversable wormhole geometry \cite{Boonserm2018}. RefG derives the metric from the compact phase-spherical branch: the active projected deficit stress of the phase-spatial medium supplies the exterior source.
...
Branch selection is set by compactness and boundary matching. For an extended
...
phase-normalized $F_{\rm min}$ sector has zero stress on the pure-phase exterior, the
diagonal field-equation residuals vanish, and the $C^2$ boundary matching
gives zero thin-shell stress.

The full dynamical selection by a finite core is the interior-and-matching
problem. The static numbers below apply to ultradense sources whose exterior
matching selects the compact phase branch.
...
\]
Thus the compact static branch gives a shadow-scale prediction
\[
\Delta b_c=+4.63\%.
\]
...
=0.931.
\]
The compact static RefG prediction is therefore the pair
\[
...
0<s_i\le 1.
\]
On the Solar $2$PN branch, the pre-repair $F_{\rm min}+C_6/Z$ determinant
derived in the local-stability section gives
$\det M_\odot(s)=5(s-1)^2$ at
$c_{Y2}=1$, $\lambda_6=1/4$, and $c_Z=1$. In this block $p_0=p_2$, so the old
determinant diagnoses the overconstraint.
The falsification filter is
therefore applied to the repaired dynamic-channel criterion,
\[
B_{\rm new}=c_Z+\epsilon_B,\qquad
M_{\rm new}=8c_{Y2}-4\lambda_6-c_Z+\epsilon_M,
\]
...
Fourth, the compact phase branch gives definite static branch predictions:
\[
r_{\rm ph}=r_s,\qquad
b_c=e\,r_s,\qquad
...
\]
which gives Newton's law in the weak spherical limit. In the Solar branch the same action admits $\gamma=\beta=1$, vanishing preferred-frame parameters, and the physical weak-Solar coefficient $q_{2{\rm PN}}=7/4$. In the tensor sector the theory has $c_g=c$.
...
phase-pressure deficit. Its null and timelike geodesic predictions are
\[
b_c=e\,r_s,\qquad
...
\]
This is the sharp static result of the compact phase branch. Its direct comparison with EHT-class images belongs to the rotating exterior and ray-tracing problem.

The present paper gives the self-contained gravitational core. The next steps are finite-core matching, the rotating compact solution, quasinormal-mode and echo stability, and observational modeling of shadows, rings, and accretion environments. Those steps determine the direct comparison with EHT, ngEHT, and BHEX data.
...
\bibitem{Yilmaz1958}
H. Yilmaz, New Approach to General Relativity, \textit{Physical Review} \textbf{111}, 1417--1426 (1958), doi:10.1103/PhysRev.111.1417.
