...
A restricted volume-response compactness class is also exhibited: for
$\chi=1$ on the principal exterior-sheet branch,
$e^{-1/2}<C_0<2/e$ implies $1<Q<2$ and fixes a stable reduced source map
$S(C_0)$.
...
residual, the source amplitude selects a static finite-core window, and a
restricted volume-response compactness class realizes $1<Q<2$. &
...
sector and its physical reading. It should not be read as a microscopic
derivation of rods, clocks, atoms, or the gravitational constant. Those are
downstream questions. In this paper the calculational input is narrower: a
...
There is one restricted class for which the selector can be written with a
single physical compactness input.  Take the volume-response attenuation
$\chi=1$ and the principal exterior-sheet branch of
$Q_0=Qe^{-Q/2}$.  Then
\[
...
branch-selection map for the stated volume-response class, while the
arbitrary-EOS and full action-coefficient map for $\chi$ remain outside the
present calculation.
...
$1<Q_\star<2$, equivalently $r_s/2<r_c<r_s$.  In the restricted
volume-response class this becomes the compactness band
$e^{-1/2}<C_0<2/e$, with
