...
\begin{abstract}
Refractive Gravity (RefG) is formulated as a metric effective field theory with
one phase scalar $\Phi$ and three spatial scalars $\phi^A$. The medium language
is used as a physical reading of this scalar sector; the calculations below use
the action, metric variation, stress balance, and branch-specific field
equations. The weak static sector gives a source-index chain from the active
stress to the refractive profile, and minimal metric coupling recovers Newton's
law for slow test bodies in the weak spherical limit. The weak-Solar branch has
$\gamma=\beta=1$, the displayed preferred-frame source combination vanishes,
and the branch optical coefficient is $q_{2{\rm PN}}=7/4$. The local tensor
sector has principal speed $c_g=c$.

The strong-field result is a realizability statement for a separate static
compact branch. On that branch the phase-normalized $F_{\rm min}$ sector is
stress-quiet, while the projected phase-pressure deficit source closes the
diagonal field-equation residual of the exponential exterior. The geodesic
numbers of this static exterior are $r_{\rm ph}=r_s$, $b_c=e\,r_s$,
$r_{\rm ISCO}=\varphi^2r_s$, and
$f_{\rm ISCO}=0.931\,f_{\rm ISCO}^{\rm GR}$ at the same mass. These are
conditional static branch markers, not direct black-hole-imaging predictions.
The deficit-feedback exponent is fixed to $\chi=1$ by the volume readout of
the asymptotic mass, so the realizing window is parameter-free:
$e^{-1/2}<C_0<2/e$, equivalently $r_s/2<r_c<r_s$. The $C^2$-matched cored
geometry is horizonless, regular, and geodesically complete, with a bounded
effective source that continues the exterior deficit.
Rotating exteriors, finite-core dynamics, QNM/echo stability, and ray-traced
plasma/accretion modeling remain separate tests.
\end{abstract}

\section{Introduction}

GR supplies the metric core of gravitational dynamics. RefG keeps this core and
adds a scalar EFT sector. The word ``medium'' is used as a shorthand physical
reading of that sector, not as a microscopic material model assumed in advance.
The objects varied below are the metric, the phase scalar $\Phi$, and the three
spatial scalars $\phi^A$.

The name ``Refractive Gravity'' refers to the weak static reading of the source
equation. Metric variation gives an active stress tensor. In a static spherical
weak field, the Bianchi/Tolman--Oppenheimer--Volkoff balance reads this stress
as a refractive profile for slow matter and light. Matter remains minimally
coupled to the metric. The result is a Newtonian source-index chain in the weak
spherical limit, not a claim of a completed nonlinear medium dynamics.

The scalar sector is built in the usual EFT way. The invariants of $\Phi$ and
$\phi^A$ define a local functional, as in scalar-tensor, gravitational EFT,
solid, and framid constructions
\cite{Horndeski1974,Gubitosi2013,BelliniSawicki2014,Deffayet2009,Kobayashi2011,Cheung2008,Endlich2013,Nicolis2015,Endlich2014,Bordin2017}.
The tensor sector is kept in the constant-$G_4$, $G_5=0$ class selected by the
observed equality of gravitational-wave and light speeds
\cite{Abbott2017GW,Abbott2017GRB,Creminelli2017,Sakstein2017}. Thus the local
principal tensor speed is luminal.

The compact part of the paper is not presented as a new metric ansatz. Similar
exponential geometries have an independent literature, including traversable
wormhole analyses \cite{Boonserm2018}; strong-field alternatives to GR provide
the comparison setting \cite{Damour1992,Damour1993,Damour1996}. The RefG
question is narrower: can the scalar EFT source realize this exponential
exterior without adding an arbitrary tensor by hand? The answer shown below is
yes for a static compact branch. The projected phase-pressure deficit has the
required sign pattern, and the phase-normalized $F_{\rm min}$ sector is quiet on
that branch.

The paper therefore separates three claims. First, the weak branch reproduces
Newtonian gravity and the displayed weak-Solar checks. Second, the local tensor
sector has $c_g=c$. Third, the compact exponential exterior is a conditional
static branch realized by a projected deficit source. The compact geodesic
numbers are markers of that branch. They are not exported as direct EHT,
ringdown, or rotating-object predictions.

\medskip

\noindent\textbf{Conventions}\par\smallskip
...
\item \textbf{Medium language.} This is a heuristic name for the scalar EFT
sector represented by $\Phi$ and $\phi^A$. The article does not assume a
microscopic substrate.
\item \textbf{Active stress.} This is the stress tensor obtained by metric
variation of the scalar-sector action. In exterior regions it is the source in
$G_{\mu\nu}=8\pi G\Theta_{\mu\nu}^{\rm RefG}$.
\item \textbf{Refractive source-index chain.} This is the weak static calculation
that reads the stress balance as
$\Pi_\Delta'\to h_{\rm eff}\to n_{\rm eff}\to a_r$.
\item \textbf{Branch.} A branch is a specified background and matching regime of
the same EFT sector. The weak-Solar branch has $H=0$; the compact static branch
has $H=H_\Delta=h$.
\item \textbf{Phase normalization.} The compact branch evaluates $F_{\rm min}$ on
$H$-rescaled invariants $\widehat Y$, $\widehat\lambda_r$, and
$\widehat\lambda_t$. The raw unnormalized compact insertion is used only as a
diagnostic obstruction.
\item \textbf{Dynamic completion.} This is the local principal-symbol repair that
opens the scalar-longitudinal subluminal window. Full curved-background
nonlinear hyperbolicity remains a separate problem.
\end{list}
...
residual, the source amplitude selects a static finite-core window with
derived feedback exponent $\chi=1$ (parameter-free band $1<Q<2$), and the
$C^2$-cored geometry is geodesically complete and regular with a bounded
junction-continuous effective source. &
No general compact-object branch-selection theorem, black-hole replacement,
EHT support, rotating exterior, QNM/echo stability, or plasma/accretion
ray-tracing claim is made here. \\
\hline
\end{tabular}
\end{center}

\section{The EFT Medium Language and the Physical Picture}

The ``base medium'' terminology is a compact way to describe the scalar EFT
sector and its physical reading. It is not a microscopic derivation of rods,
clocks, atoms, or the gravitational constant; those are downstream questions. In this paper the calculational input is narrower: a
metric action, scalar invariants, and the stress tensor obtained by metric
variation.

Within this EFT reading, the phase, pressure, and spatial deformation variables
are separate response channels of the same scalar sector. A localized weak
source is represented by an active stress whose static balance defines an
effective refractive profile. In the weak spherical regime this profile gives
Newton's force. In the compact sector, a separate source/load condition selects
the phase-dominated static branch that realizes the exponential exterior.
...
The fields $\Phi$ and $\phi^A$ represent different response channels of the
same scalar EFT sector. The $YI_1$ term couples these channels while keeping the
phase response and the spatial response as separate dynamical properties. This
distinction is central in the scalar-longitudinal stability criterion below.
...
through the Solar order displayed here, rather than a formal reduction of the action alone.
...
weak-Solar metric branch; the fully coupled self-consistent exterior ODE and a
full raw Solar-System likelihood remain separate steps. RefG changes the source
mechanism, while the displayed weak-Solar $1$PN/$2$PN optical coefficients remain
GR-compatible.
...
The exponential exterior has no finite-radius event horizon in the static chart.
Its strong-field use is therefore tied to tests of horizonless-candidate
ultracompact objects. For such candidate objects, a photon sphere has both a
shadow-scale role and a dynamical-stability role: the light-ring structure is a
stability criterion in its own right \cite{Cunha2017}, and the ringdown/echo
channel gives a gravitational-wave discriminator from black holes
\cite{Cardoso2016}. The quasinormal-mode (QNM)/echo channel is therefore an
open astrophysical-viability risk and part of the strong-field program itself.
The result checked here is the static geometric core. Its astrophysical use
proceeds through finite-core dynamics, the rotating exterior, QNM/echo analysis,
and Event Horizon Telescope (EHT), next-generation Event Horizon Telescope
(ngEHT), and Black Hole Explorer (BHEX) ray tracing.
...
diagonal field-equation residuals vanish. A smooth $C^2$ exterior matching
introduces no thin-shell stress at the boundary, and the matched object is a
complete geometry. The $C^2$ logarithmic core profile on $r<r_c$, with
$\log(AB)=q\,[(r/r_c)^2-1]^3$ for $q=r_s/r_c$, has a regular isotropic center
with finite curvature, removes the infinite proper radial depth of the bare
exterior, and lets radial null geodesics reach the center at finite affine
parameter and extend through it. Its effective source is bounded on the core
and joins the exterior continuously, with the radial null load equal to
$-2\Delta_P$ at $r_c$. The cored geometry is therefore horizonless, regular,
and geodesically complete, and the geodesic numbers below are markers of this
...
bistable fold. For the finite-core compact exterior used by the geodesic markers
the required window is
\[
1<Q_\star<2,\qquad
...
compact $H=h$ exterior; the object-specific value of $S$ belongs to the
finite-core equation of state or collapse history, while the exponent $\chi$
is fixed next.

The exponent $\chi$ is set by the readout channel, not by the polynomial
coefficients: the phase-normalized $F_{\rm min}$ sector is stress-quiet on
this branch, so $\chi$ carries no dependence on the $c_i$ or on
$\omega_\Delta$, and it is the exponent of the deficit-readout filter in the
volume variable $q_v=3h$. The external source amplitude is the ADM/Komar
mass, a volume-extensive charge read through the filter $e^{-3h}=e^{-q_v}$,
which gives $\chi=1$; the intensive clock/rod filter $e^{-h}=e^{-q_v/3}$,
with exponent $1/3$, is not the gravitating channel. With $\chi=1$ and the
principal exterior-sheet branch of $Q_0=Qe^{-Q/2}$, the selector takes a
single physical compactness input:
\[
Q(C_0)=-2W_0\!\left(-\frac{C_0}{2}\right),\qquad
q(C_0)=\frac{3Q(C_0)}{2},
\]
and the compact finite-core window is exactly
\[
e^{-1/2}<C_0<\frac{2}{e}
\quad\Longrightarrow\quad
1<Q(C_0)<2.
\]
The reduced source map is then
\[
S(C_0)=q(C_0)e^{q(C_0)},
\]
so the balance $\dot q=S(C_0)e^{-q}-q$ has the fixed point
$q=q(C_0)$ and linear eigenvalue $-(1+q(C_0))<0$.  This closes the reduced
branch-selection map with a derived exponent: $\chi$ has no coefficient
dependence to absorb, the volume channel fixes $\chi=1$, and the window is
parameter-free.  An arbitrary-EOS, relativistic-TOV compactness map remains
outside the present calculation.

As an explicit equation-of-state realizing class, the Newtonian $n=1$
polytrope
\[
p=K\rho_c^2
\]
has compactness
\[
C_0=\frac{4K\rho_c}{c^2}.
\]
It enters the compact window when
\[
e^{-1/2}<\frac{4K\rho_c}{c^2}<\frac{2}{e}.
\]
This is a realizing class and compactness band, not a relativistic TOV fit or
an observational EOS claim.
...
window:
\[
Q_\star=\frac{2W(S)}{3},\qquad
1<Q_\star<2,
\]
with the derived feedback exponent $\chi=1$, equivalently
\[
\frac32 e^{3/2}<S<3e^{3},\qquad
\frac{r_s}{2}<r_c<r_s.
\]
...
Q_\star=\frac{2W(S)}{3},
\]
with the feedback exponent fixed to $\chi=1$ by the volume readout of the
asymptotic mass.  Finite-core exterior applications require $1<Q_\star<2$,
equivalently the parameter-free window $r_s/2<r_c<r_s$, realized on a
horizonless, regular, geodesically complete cored geometry.  On the
principal exterior-sheet branch this is the compactness band
$e^{-1/2}<C_0<2/e$, with
$S(C_0)=q(C_0)e^{q(C_0)}$ and
$Q(C_0)=-2W_0(-C_0/2)$.  The $n=1$ polytropic realizing class
$p=K\rho_c^2$ enters this band when
$e^{-1/2}<4K\rho_c/c^2<2/e$.  Direct comparison with EHT-class images belongs
to the rotating exterior and ray-tracing problem.

The present paper gives the checkable article-level gravitational core. The
next steps are arbitrary and relativistic finite-core equations of state, the
rotating compact solution, quasinormal-mode and echo stability, and
observational modeling of shadows, rings, and accretion environments. Those
steps determine the direct comparison with EHT, ngEHT, and BHEX data.
...
The algebraic checks underlying the validation table are implemented in the
accompanying Python/SymPy source tree under \texttt{RG/work/}. The relevant
check scripts are the \texttt{p01}, \texttt{p03}, \texttt{p04}, \texttt{p05},
\texttt{p13}, and \texttt{p16} sector gates. These checks verify symbolic identities and
branch-scope gates; they do not constitute an observational likelihood fit, a
rotating compact-object solution, or a numerical plasma/accretion simulation.
