...
Refractive Gravity ($\RefG$) takes a relational principle of measurement as its starting point: clocks, rulers, light, and observers belong to the same physical world, and measurements are made using material standards within that world. On this basis, the monograph develops a unified ontological account of a single foundation and the geometry, matter, and time that emerge from it. In the specified low-energy limit, the same relational coframe supports Einstein--Hilbert geometry, while the quadratic sector of the oscillon action about its strict vacuum is a free complex scalar quantum field on that coframe. The text presents the conceptual analysis and physical interpretation; the published formalism and calculations are available in the public scientific archive on Zenodo \citep{lotishvili2026a,lotishvili2026b}.
...
\item \textbf{The Koide $C_3$ Phase Geometry:} A three-phase map in charged-lepton ($e,mu,	au$) square-root mass space reproduces Koide's $K=2/3$ identity exactly when $A_K=\sqrt2$. $\RefG$ reads this geometry as a candidate imprint of a ninth-order return cycle in which $h=2$ fixes the phase angle $\theta=2/9$.
...
\begin{enumerate}[label=\arabic*.]
\item \textbf{The Absolute Maximum of Substance:} A limiting state characterized by maximal pressure, mass--energy potential, and operational density.
...
The primary unit of measurement is a preserved relation. In a completely undifferentiated state, no second term is available for comparison. Self-distinction yields two distinct states of the same foundation and establishes the primary link between them; here begin counting, spatial extension, and the relative ordering of events.
...
\begin{itemize}
\item At scales smaller than $\ell_*$, no independent measurable relation has yet emerged; information remains indistinguishable;
...
$\RefG$ describes this relation through transmission between foundation nodes: rarefaction reduces the frequency of collisions and information exchange. Wave states are consequently transmitted more slowly, reducing the coordinate clock rate.
...
\caption{\textbf{Schematic illustration of the operational readout of an oscillon within a pressure-deficit zone.}
...
Self-regulation in the foundation-medium couples the state of matter, the pressure deficit, and the response of local clocks and rulers. Their joint dynamics require a common geometry in which each energy contribution from matter and the medium is counted once.
...
Within the equilibrium family computed by coupling the canonical oscillon field to Einstein geometry, the total mass of a bound system is smaller than the summed masses of isolated configurations carrying the same total phase charge. Equal increments of charge yield progressively smaller increments of external mass, while the central proper clock slows relative to a distant clock. The geometric determination of the external mass agrees with the full energy accounting, including binding. Total mass is thus a property of the jointly adjusted configuration.
...
This self-consistent equilibrium has a concrete dynamical response. A weak inward perturbation that conserves phase charge first compresses the compact configuration, then arrests the contraction and produces expansion; the body continues to oscillate over the computed interval. Matter motion, the gravitational field, and the local clock rate evolve together under equations derived from a single action.

Stronger compression in the same model produces a trapped-light region: spherical light fronts sent in either radial direction decrease in area. The calculation continues beyond the formation of this region, tracking central proper time, density, and curvature together. The trapping geometry and the physical evolution of the interior are thus described by one dynamical solution (Figure~\ref{fig:strong-field-process}).

\begin{figure}[p]
\centering
\includegraphics[width=\linewidth,height=0.86\textheight,keepaspectratio]{figures/strong_field_process_EN.pdf}
\caption{\small Strong-field numerical results for the coupled canonical scalar--Einstein model. A--C show the mass, two bulk-radius measures, and central clock rate of four equilibria; $Q/Q_0$ denotes multiples of a reference phase charge. D shows contraction and recovery following a weak perturbation; E--F show the response to a stronger perturbation. In the shaded region, both future-directed radial light fronts decrease in area. Quantities are expressed in dimensionless model units; $t$ and $T$ are different time coordinates. The dynamical curves cover finite computed time intervals.}
\label{fig:strong-field-process}
\end{figure}

In a description of a regular interior, the energy balance of the foundation and matter, the local clock rate, and the spatial scale are determined jointly. Smooth matching to the exterior geometry must follow from the same dynamics. Local measurements use clocks and rulers formed in the same environment; comparison with a distant observer reveals the difference between their scales.

Assessing singularity avoidance requires the local source, curvature measured in freely falling frames, and geodesic extendibility. Penrose's null-convergence condition $R_{\mu\nu}k^\mu k^\nu\ge0$ is related to the null energy condition (NEC) through Einstein's equation and must be considered together with the theorem's other global premises \citep{penrose1965}. A stable phase current satisfying $\rho_C+p_C>0$ remains an NEC-compatible source within its own action; the contribution of the additional medium source is determined by its separate variation.
...
The argument also has a precise empirical formulation in terms of \textbf{exact structural correspondences}:
\begin{itemize}
...
\item the precise pattern of fractional quark charges \citep{navas2024}.
\end{itemize}
...
\item An interior-core model for a compact object fails to match Event Horizon Telescope shadow diameters, the compact-branch oscillation spectrum, or gravitational-wave echo limits \citep{cardoso2019}.
...
This section brings together the results obtained, their conditions of validity, and the physical questions that remain to be resolved.
...
\item \textbf{A Common Local Frequency Scale}---The relations $\Omega_t/\Omega_{t,0}=p$ and $d\tau=p\,dt$ provide the exact dictionary between coordinate and process time. The particle decomposition $\nu_i^{(t)}=s_i\Omega_t$ is an independent spectral hypothesis: the environment fixes the common frequency scale $\Omega_t$, while the internal structure of the oscillon fixes the dimensionless labels $s_i$. Establishing this spectral hypothesis requires constructing the perturbation operator of a finite-energy oscillon and extracting the common factor $\Omega_t$ from its eigenspectrum.

\item \textbf{Oscillon Deficit, Size, Time, and Mass}---The metric filters and the first-order biconformal branch have been obtained symbolically. On this branch, operational size, process cycle rate, and externally inferred mass follow the same $p$ response. A minimal coframe-coupled complex $U(1)$ action, its analytic existence window, and a finite-energy Q-ball-type core with a radially nodeless profile on a fixed Minkowski coframe have been obtained, with convergent numerical results supporting orbital stability. Coupled solutions of this same canonical field and Einstein geometry already incorporate gravitational backreaction, equilibrium, and an oscillatory response to specified perturbations. Further physical identification concerns the exact relation between the core's internal energy and the pressure deficit, operational size, process cycle rate, rest mass, and pole mass.
...
The complete 1PN result belongs to the selected TEGR coframe class. The galactic, free-quantum, and CMB results retain their respective actions and source conditions; incorporating them into a unified dynamical description requires counting each relevant source once and consistently matching backgrounds and perturbations.
...
\item \textbf{Inertia and Gravitational Mass}---For a finite-energy Lorentz-covariant localized solution satisfying the Laue boundary condition $\int T^{ij}\,d^3x=0$ for all $i,j$, the Noether identity relates the inertial coefficient and the gravitational source to the same total energy of the system. The microscopic justification of this relation requires deriving it directly from the oscillon's translational zero mode.
...
\item \textbf{Compact Objects and Strong Fields}---In the canonical oscillon--geometry sector, the nonadditivity of the total mass, a recovery response after small compression, and the formation of a light-trapping region under stronger compression have been obtained. Compression and curvature growth continue over the tested portion of the interior. Incorporating the full dynamical response of the foundation and obtaining a self-regulated, singularity-free black-hole solution remain research objectives.
...
\item \textbf{Particles as Oscillon Modes and the Scalar Quantum-Field Bridge}---A local nonlinear amplitude--phase action for the oscillon and a finite-energy spherical Q-ball-type candidate core with numerical evidence of orbital stability have been obtained. The vacuum quadratic sector of the same action takes exactly the form of the free complex Klein--Gordon action. On a fixed Minkowski coframe, standard canonical quantization of this quadratic sector gives the free excitations about the strict vacuum a bounded-below, nonnegative free Hamiltonian, bosonic Fock space, particle and antiparticle sectors, a global $U(1)$ phase charge, microcausality, a propagator pole on the mass shell, the spin-$0$ Casimir, and a controlled Schrödinger limit. This is the first exact $\RefG$ bridge to standard quantum field theory within the same coframe that yields Einstein--Hilbert geometry.

A particle description of the nonlinear localized core requires its collective quantization and identification as a one-particle, bound, or coherent state. Related physical questions include the dynamics of population interlocking, the full spectrum of stability and decay channels, experimental bounds on the proton-decay lifetime, tests of the common frequency factor and the $t/\tau$ dictionary, and the transition from rest mass to interacting pole mass. The origin of $\hbar$, canonical commutators, Hilbert space, the Born rule, and the vacuum choice from the foundation constitutes a separate quantum-foundation bridge. The global phase charge remains an internal $U(1)$ charge, and the free spin-$0$ quantum remains a free excitation about the strict vacuum.
...
\item \textbf{Spin, Pauli Exclusion, and Fermionic Statistics}---The Möbius $Z_2$ double cover exactly reproduces the kinematics of a sign change under $2\pi$ and return to the initial state under $4\pi$; it supplies the topological structure of the $4\pi$ spinorial kinematics associated with spin-$1/2$. A complete spinorial description requires the joint derivation of an $SU(2)$ representation, a Lorentz spinor, and a covariant action that recovers the Dirac equation and the factor $g=2$.
...
\nocite{abdulkarim2025,brout2022,mandelbrot1982,milne1935,milne1948,planck2020,michell1784,kleinert2008}
