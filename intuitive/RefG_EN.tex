...
\hypersetup{
    hidelinks,
    pdfstartview={FitH},
    pdftitle={Refractive Gravity: An Ontological Picture and the Foundations of Mathematical Formalism (Version 3.1)},
    pdfauthor={George Lotishvili}
}

\newcommand{\RefG}{\mathrm{RefG}}

\title{\textbf{Refractive Gravity: An Ontological Picture and the Foundations of Mathematical Formalism}\\[0.5em]\large Version 3.1}
...
The unified $\RefG$ picture rests on seven pillars:
\begin{enumerate}[label=\arabic*.]
...
Standard field-theoretic oscillons are long-lived but gradually decay through radiative leakage. $\RefG$ offers population-level phase locking as the mechanism that suppresses this leakage and selects stable particle modes.
...
The covariant action of a particle sector must provide the exact bridge between its node-scale topological form, its Hilbert stress--energy tensor, and the pressure geometry it generates. Across this bridge, spin, electric charge, color, and polarization retain their distinct quantum roles, while their complete energetic contribution is read once by the common metric.
...
At the particle level, the volumetric resonator, common cadence, and topological invariants form a unified geometric map of mass, spin, charge, color, and generations. The decisive test of this map begins with the dynamics of one finite-energy oscillon: its eigenspectrum must connect the microdynamics of the foundation to measurable particle masses and quantum numbers.
...
\item \textbf{Particles as Oscillon Modes}---The minimal complete computational unit of the particle sector is one oscillon family. A two-level filter describes mode selection first by kinematic and population locking, then by energetic stability. The electron's observed stability and the lower bound on the proton lifetime, $\tau_p>10^{34}$ years, provide stringent empirical targets for the proposed topological-protection mechanism; $\RefG$ associates that mechanism with conserved charges and population phase locking.

The complete particle derivation comprises a local nonlinear action, a localized finite-energy solution, the dynamics of population interlocking, the spectrum of stability and decay channels, calculation of the proton lifetime, the perturbation operator, tests of common frequency factorization and of the $t/\tau$ dictionary, and an independent map from energy to rest mass and pole mass.
