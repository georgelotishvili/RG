...
This mechanism is jointly testable against the SPARC/RAR relation \citep{lelli2016}, galaxy-cluster mass budgets, Bullet-Cluster-type mergers, the CMB, BAO, and large-scale-structure statistics. Taken together, these channels determine whether one vortical pressure response describes all of these scales.
...
A covariant phase-current action connects this conservation law to the post-Genesis coframe geometry. With the $(-+++)$ signature, variation of $\theta_C$ gives $\nabla_\mu J^\mu=0$, while metric variation gives the Hilbert stress--energy tensor,
\begin{equation}
p_C=n_C\rho_C'(n_C)-\rho_C(n_C),
\qquad
T^C_{\mu\nu}=(\rho_C+p_C)u_\mu u_\nu+p_Cg_{\mu\nu}.
\end{equation}
This action describes a barotropic, isentropic, irrotational subfamily governed by one phase potential. The oscillon microphysics enters through $\rho_C(n_C)$, while the constitutive relation between $p_C$ and the foundation background pressure $P_F$ turns phase-current conservation into the background relaxation law.
