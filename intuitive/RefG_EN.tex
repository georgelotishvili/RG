...
Consider a wave signal propagating between regions with different pressure states, for example from a gravitational well to a distant observer. The photon follows the operational geometry $A=a/p$, and the receiver compares its phase rate with a local atomic standard. For ideal transparent propagation, this gives
\begin{equation}
\frac{\omega_o}{\omega_e}=\frac{A_e}{A_o},
\qquad
1+z=\frac{A_o}{A_e}=\frac{a_o/p_o}{a_e/p_e}.
\end{equation}
The common response of the signal and atomic standard to pressure is already incorporated in the $A$ geometry. At each endpoint, the phase ratio is expressed relative to the local frequency scale $\Omega_t$.
...
The resulting feedback is bidirectional: \textbf{an oscillon produces a pressure deficit through its internal oscillation, and that deficit in turn determines its effective mass, operational size, and trajectory through the medium.} Matter changes the medium's pressure state, and the altered medium determines the geometry of refractive motion.
