import sympy as sp
import json
import hashlib
from pathlib import Path

def generate_hash(content):
    if not content:
        return ""
    return hashlib.sha256(content.encode('utf-8')).hexdigest()

def main():
    print("Running W3_19b: Non-Linear Coupling & Matter Epoch Recovery...")
    
    # Define symbols
    A, a_e, t, r, c = sp.symbols('A a_e t r c', real=True, positive=True)
    kappa_tau, beta_G, rho_m0, P_ref = sp.symbols('kappa_tau beta_G rho_m0 P_ref', real=True, positive=True)
    
    # ---------------------------------------------------------
    # 1. Kinematic Bridge and Clock Cancellation
    # ---------------------------------------------------------
    p_e = a_e**(-sp.Rational(3, 4))
    p_o = 1
    
    # Biconformal metric: ds^2 = -c^2 p^2 dt^2 + p^-2 a^2 dr^2
    # Coordinate time stretch for photon propagation:
    dt_stretch = (1 * p_o**(-2)) / (a_e * p_e**(-2))
    dtau_stretch = dt_stretch * (p_o / p_e)
    dtau_stretch = sp.simplify(dtau_stretch)
    
    # Physical scale factor A = a/p
    A_e = a_e / p_e
    Z_geom = 1 / A_e - 1
    
    sn_kinematic_cancellation = bool(sp.simplify(dtau_stretch - (Z_geom + 1)) == 0)
    print(f"Biconformal kinematics identical to 1+Z: {sn_kinematic_cancellation}")
    
    # ---------------------------------------------------------
    # 2. Physical Closure & H_phys Derivation
    # ---------------------------------------------------------
    # Geometric readout map (from RefG baseline):
    # P_A = P_ref * A^(-6/7)
    P_A = P_ref * A**(-sp.Rational(6, 7))
    
    # Postulate 1: Physical dust matter conservation
    rho_m = rho_m0 * A**(-3)
    
    # Postulate 2: RefG NON-LINEAR evolutionary dynamics
    # dP/dtau = - kappa_tau * P - beta * P * sqrt(G * rho_m)
    # We combine beta * sqrt(G) into a single parameter beta_G
    dP_dtau_RHS = - kappa_tau * P_A - beta_G * P_A * sp.sqrt(rho_m)
    
    # Chain rule: dP/dtau = (dP/dA) * A * H_phys
    dP_dA = sp.diff(P_A, A)
    chain_rule_LHS = dP_dA * A 
    
    # H_phys = (dP/dtau) / (A * dP/dA)
    H_phys = sp.simplify(dP_dtau_RHS / chain_rule_LHS)
    
    print(f"Derived H_phys(A) = {H_phys}")
    
    # ---------------------------------------------------------
    # 3. Matter Epoch Recovery Gate (q_early)
    # ---------------------------------------------------------
    # Deceleration parameter q = -1 - (A/H) * (dH/dA)
    dH_dA = sp.diff(H_phys, A)
    q_A = sp.simplify(-1 - (A / H_phys) * dH_dA)
    
    # Evaluate early limit (A -> 0)
    q_early = sp.limit(q_A, A, 0)
    print(f"Deceleration parameter in early limit q_early = {q_early}")
    
    # The gate passes ONLY if q_early == 1/2 (standard matter era)
    matter_epoch_recovered = bool(sp.simplify(q_early - sp.Rational(1, 2)) == 0)
    print(f"Matter epoch (q=1/2) recovered: {matter_epoch_recovered}")
    
    # Evaluate late limit (A -> oo)
    q_late = sp.limit(q_A, A, sp.oo)
    print(f"Deceleration parameter in late limit q_late = {q_late}")
    
    # ---------------------------------------------------------
    # 4. Structural Growth Integral Formulation
    # ---------------------------------------------------------
    A_e, A_f = sp.symbols('A_e A_f', real=True, positive=True)
    Gamma_tau = sp.Function('Gamma_tau')(A)
    
    dN_dlnA = Gamma_tau / H_phys
    N_RefG = sp.Integral(dN_dlnA / A, (A, A_f, A_e))
    
    print(f"Formulated Structural Integral N_RefG(A_e, A_f) = {N_RefG}")
    
    # ---------------------------------------------------------
    # 5. Save State and Hashes
    # ---------------------------------------------------------
    script_path = Path(__file__)
    script_hash = generate_hash(script_path.read_text('utf-8'))
    
    prereg_path = Path(__file__).parent / "w3_19b_nonlinear_coupling_preregistration.md"
    prereg_content = prereg_path.read_text('utf-8') if prereg_path.exists() else ""
    prereg_hash = generate_hash(prereg_content)
    
    if matter_epoch_recovered:
        status = "CONDITIONAL PASS (MATTER EPOCH RECOVERED)"
        conclusion = (
            "W3_19b successfully implemented the nonlinear coupling dP/dtau = -kappa*P - beta*P*sqrt(G*rho_m). "
            "This is a physically motivated constitutive postulate (based on the Jeans/free-fall frequency), not yet a microscopic derivation. "
            "This derived H_phys(A) = 7*kappa_tau/6 + (7*beta_G*sqrt(rho_m0)/6) * A^{-1.5}. "
            "This functional form algebraically yields q=1/2 in the matter-dominated limit, rigorously recovering the matter epoch, "
            "and q_late = -1, yielding a stable late-time accelerating epoch (de Sitter). "
            "The kinematic tests still show 1+Z identical cancellation. "
            "The pipeline is now completely ready for the background observational fit (W3_20). "
            "Note: The functional form H(z) = K + B(1+z)^{1.5} returns to the old baseline, which historically had Pantheon tensions. "
            "Therefore, W3_20 MUST begin with a frozen CC+Pantheon+BAO check before any JWST claims are made."
        )
    else:
        status = "FAIL"
        conclusion = "Matter epoch failed."

    print(f"\n--- GATE STATUS: {status} ---")
    print(conclusion)
    
    result_data = {
        "claim_id": "W3_19B_NONLINEAR_COUPLING_BRIDGE",
        "status": status,
        "source_hashes": {
            "script_hash": script_hash,
            "preregistration_hash": prereg_hash
        },
        "checks": {
            "biconformal_kinematics_cancel": sn_kinematic_cancellation,
            "matter_epoch_recovered": matter_epoch_recovered,
            "JWST_decoupling_observationally_proven": False,
            "SN_BAO_re_evaluated_in_A_tau": False
        },
        "equations": {
            "H_phys_A": str(H_phys),
            "q_early_limit": str(q_early),
            "q_late_limit": str(q_late),
            "N_RefG_integral": str(N_RefG)
        },
        "conclusion": conclusion
    }
    
    out_file = Path(__file__).parent / "w3_19b_result.json"
    with open(out_file, "w") as f:
        json.dump(result_data, f, indent=2)

if __name__ == "__main__":
    main()
