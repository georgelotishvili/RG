...
```text
p_n(x)^2 = n_C(x)/n_infinity,          n_infinity>0,
```

and asks whether the same positive density can reproduce the decreasing
static operational factor outside positive mass.
...
`c_s^2=-1/2`, outside the stable W3-54 branch.  The endpoint `c_s^2=0`
cannot support a regular varying-lapse equilibrium because `mu` is locally
constant.

The inverse-density control `p_inv^2=n_infinity/n_C` has the correct static
sign and matches the linear coefficient at `c_s^2=1/2`, but it reverses the
...
the homogeneous density map again forces `c_s^2=-1/2`.
...
Use the W3-54 covariant phase-current action and conventions,
...
```text
n_C=n_infinity calN^2,
mu(n_C)=mu_infinity (n_C/n_infinity)^(-1/2),
d ln(mu)/d ln(n_C)=-1/2,
rho_C''<0.
```
...
1. With `p_inv^2=n_infinity/n_C`,
   `delta ln(p_inv)=-u/(2c_s^2)`; the static coefficient matches at the
   healthy value `c_s^2=1/2`.  This map has
   `d ln(p_inv)/d ln(n_C)=-1/2`, opposite to the frozen homogeneous
   W3-50 derivative `+1/2`.
2. With `p_mu=mu_infinity/mu`, the stationary equation gives `p_mu=calN`
   exactly.  Compatibility with `p^2=n_C/n_infinity` over the same
   one-variable constitutive branch requires
   `d ln(mu)/d ln(n_C)=-1/2`, the same excluded value.
...
branch, the universal local extension `p_n^2=n_C/n_infinity` cannot equal
...
homogeneous density-map derivative require `c_s^2=-1/2`, while W3-54
...
`W3-70-v1.0-COLLECTIVE-PHASE-CARRIER-ADMISSIBILITY`.
...
2. W3-50 supplies a positive collective density `n_C`, its exact diagonal
   shift current, and the homogeneous map
   `eta_F=n_C/n_C0`, `p^2=eta_F`.
...
4. The candidate is the new universal local extension
   `p_n^2=n_C/n_infinity`; it is tested rather than assumed true.
...
- new fields, metrics, sources, action operators, fitted parameters and fitted
  functions: zero;
- candidate choice: one local extension `p_n^2=n_C/n_infinity`, frozen
  before evaluation;
...
3. Express the W3-54 sound speed as the logarithmic slope
   `d ln(mu)/d ln(n_C)`.
4. Combine the Bernoulli residual
   `c_s^2 d ln(n_C)+d ln(calN)=0` with the candidate residual
   `d ln(n_C)-2d ln(calN)=0`.
5. Solve the joint residuals on a nonconstant exterior and compare the result
   with the frozen health interval.
6. Evaluate the weak positive-mass sign independently.
7. Run the inverse-density and chemical-potential controls.
8. Audit one-metric/one-source preservation, package cleanliness, immutable
   CODES/intuitive controls, and false scope flags.
9. Apply every mutation through the same production validators.
...
residuals jointly require `c_s^2=-1/2` on every nonconstant branch; that
...
control, package, immutable file, or mutation differs from the registration;
...
- candidate residual:
  `R_p=d ln(n_C)-2 d ln(calN)=0`.
- joint nonconstant-branch residual:
  `(2c_s^2+1)d ln(calN)=0`.
- health residual at the matched candidate point:
  `c_s^2+1/2=0`, with `c_s^2=-1/2` outside `[0,1]`.
...
`rho_C'>0`, `0<=c_s^2<=1`.  The required `c_s^2=-1/2` is a gradient-
...
- `INVERSE_DENSITY_CONTROL`: healthy static sign at `c_s^2=1/2`, but a
...
`p_n^2=n_C/n_infinity`.  They do not identify a unique replacement.
...
d ln(n_C)=2 d ln(calN)           [tested candidate],
c_s^2_required=-1/2,
c_s^2_inverse=+1/2,
p_mu=mu_infinity/mu=calN,
d ln(p_homogeneous)/d ln(n_C)=+1/2,
d ln(p_inverse)/d ln(n_C)=-1/2.
```
...
`p_n=calN<1` and `p_n^2=n_C/n_infinity` require
